function getViewingBehaviorSFT(FIXATIONFILE,SAMPRATE,imageX,imageY,PLOTOPTIONS)
...
densitymap = zeros(imageY,imageX);
    ...
    if ~isempty(fixations)
        ...
        for i = 1:size(fixations,2)-1
            ...
            if i == 1
                ...
                x = fixations(1,i)-imageX/2;
                y = fixations(2,i)-imageY/2;
...
fixcount = 1;
saccount = 1;
for cndlop = 1:length(fixationstats); %only uses novel viewing since memory could alter natural behavior
    fixationtimes = fixationstats{cndlop}.fixationtimes;
    if ~isempty(fixationtimes)
        ...
        for i = 1:size(saccadetimes,2);
            stimes = saccadetimes(1,i)-5:saccadetimes(2,i)+7;  
            stimes(stimes < 1) = [];
            stimes(stimes > size(xy,2)) = [];
            x = xy(1,stimes);
            y = xy(2,stimes);
            velx = diff(x);
            vely = diff(y);
        ...
        for i = 1:size(fixationtimes,2);
            ftimes = fixationtimes(1,i)-5:fixationtimes(2,i)+7;
            ftimes(ftimes < 1) = [];
            ftimes(ftimes > size(xy,2)) = [];
            x = xy(1,ftimes);
            y = xy(2,ftimes);
            velx = diff(x);
            vely = diff(y);
...
if strcmpi(PLOTOPTIONS,'all')
    ...
    figure
    hist(1000./(timebtwfix(~isnan(timebtwfix))*5),100)
    title('Fixation Rate')
    xlabel('Hz')
