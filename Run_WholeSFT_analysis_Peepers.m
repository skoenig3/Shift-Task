...
% Similar to RunWholeSalienceModel written by Seth Konig
...
for imset = 1:length(image_sets);
    ...
    for i = 1:120
        if i < 10
            load(['00' num2str(i) '-saliencemap'],'fullmap')
        elseif i < 100
...
save('C:\Users\seth.koenig\Documents\MATLAB\Shift Task\Combined-Peepers-Salience',...
...
save(['C:\Users\seth.koenig\Documents\MATLAB\Shift Task\'...
    'CombinedViewingBehavior-Peepers.mat'],'tags','allview',...
...
Combinedbehaviorfile = ['C:\Users\seth.koenig\Documents\MATLAB\Shift Task\'...
    'CombinedViewingBehavior-Peepers.mat'];
...
for i = 1:9;
    ...
    centroids{i} = NaN(120*length(image_sets),2);
end
img = NaN(9,120*length(image_sets));
center = NaN(9,120*length(image_sets));
...
for SET = 1:length(image_sets);
    ...
    for i = 1:120;
        for t = 1:length(tags);
            if i < 10
                load([tags{t} '-00' num2str(i) '-CB.mat'],'fixationtimes');
            elseif i < 100
